...
bool auxiliar_ads_transmit(uint8_t slaveAddres, uint8_t *word,uint16_t nroBytes) {
   return FUNCTION_OK;
}
...
//Inicializar configuración de canales A1 en modo single - lectura por consulta
void test_iniciar_canal_modo_diferential(void){
    signalADS1115 channelUno;

    ADS1115_Transmit_fake.custom_fake = auxiliar_ads_transmit;
